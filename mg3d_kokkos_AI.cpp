...
/// Threshold: levels with fewer total points than this run serial host loops
/// to avoid GPU kernel-launch overhead dominating tiny grids.
static constexpr int SERIAL_THRESHOLD = 4096;  // e.g. 16^3
...
struct MultiGrid3D {
    ...
    std::vector<DeviceView> tL;  //!< level-wise double-buffer scratch (used by fused Jacobi)
    ...
    // ─────────────────────────────────────────────────────────────────────
    //  Serial (host) fallback helpers for small coarse grids
    //  Avoids GPU kernel-launch overhead when N < SERIAL_THRESHOLD.
    // ─────────────────────────────────────────────────────────────────────
    ...
    /** \brief Host-serial restriction for tiny grids. */
    static void restrict_serial(const Level3D& fine, const DeviceView& r_f, const Level3D& coarse,
                                DeviceView& f_c) {
                    ...
                    for (int dk = -1; dk <= 1; ++dk)
                        for (int dj = -1; dj <= 1; ++dj)
                            for (int di = -1; di <= 1; ++di) {
                                ...
                                sum += w * rf_h(id3(i + di, j + dj, k + dk, nxf, nyf));
                            }
                    fc_h(id3(I, J, K, nxc, nyc)) = sum / 64.0;
                }
        Kokkos::deep_copy(f_c, fc_h);
    }

    /** \brief Host-serial prolongation for tiny grids. */
    static void prolong_add_serial(const Level3D& coarse, const DeviceView& u_c,
        ...
        for (int K = 0; K < nzc - 1; ++K)
            for (int J = 0; J < nyc - 1; ++J)
                for (int I = 0; I < nxc - 1; ++I) {
                    double c000 = uc_h(id3(I, J, K, nxc, nyc));
                    double c100 = uc_h(id3(I + 1, J, K, nxc, nyc));
                    double c010 = uc_h(id3(I, J + 1, K, nxc, nyc));
                    double c110 = uc_h(id3(I + 1, J + 1, K, nxc, nyc));
                    double c001 = uc_h(id3(I, J, K + 1, nxc, nyc));
                    ...
                    uf_h(id3(i + 1, j + 1, k, nxf, nyf)) += 0.25 * (c000 + c100 + c010 + c110);
                    uf_h(id3(i, j, k + 1, nxf, nyf)) += 0.5 * (c000 + c001);
                    uf_h(id3(i + 1, j, k + 1, nxf, nyf)) += 0.25 * (c000 + c100 + c001 + c101);
                    uf_h(id3(i, j + 1, k + 1, nxf, nyf)) += 0.25 * (c000 + c010 + c001 + c011);
                    uf_h(id3(i + 1, j + 1, k + 1, nxf, nyf)) +=
                        0.125 * (c000 + c100 + c010 + c110 + c001 + c101 + c011 + c111);
                }
        Kokkos::deep_copy(u_f, uf_h);
    }

    // ─────────────────────────────────────────────────────────────────────
    //  Parallel (device) kernels — only used on large grids
    // ─────────────────────────────────────────────────────────────────────
     ...
     *        Boundary entries of out are assumed to be zero from allocation and are never
     *        overwritten, so we skip the full deep_copy(out,0) that was here before.
     */
    static void apply_A(const Level3D& lev, const DeviceView& u, const DeviceView& out) {
        if (lev.N() < SERIAL_THRESHOLD) {
        ...
        const double ihz2 = 1.0 / (lev.hz * lev.hz);
        const double diag = 2.0 * ihx2 + 2.0 * ihy2 + 2.0 * ihz2;
        // NOTE: no deep_copy(out,0) — boundary entries stay zero from allocation.
        Kokkos::parallel_for(
    ...
    /** \brief Compute residual \f$r = f - Au\f$ in a single fused kernel (no temporary).
     *  \param lev level geometry
     *  \param u current iterate
     *  \param f right-hand side
     *  \param r output residual
     *  \param Au_scratch unused (kept for interface compatibility)
     */
    static void residual(const Level3D& lev, const DeviceView& u, const DeviceView& f,
        ...
        const double ihz2 = 1.0 / (lev.hz * lev.hz);
        const double diag = 2.0 * ihx2 + 2.0 * ihy2 + 2.0 * ihz2;
        // Fused: r = f - A*u in one pass. No temporary Au needed.
        // Boundary entries of r stay zero (Dirichlet: f=0, u=0 on boundary).
        Kokkos::parallel_for(
            "residual_fused",
    ...
    /** \brief Fused weighted-Jacobi smoother: \f$u \leftarrow u + \omega D^{-1}(f-Au)\f$.
     *  \param lev level geometry
     *  \param u in/out iterate
     *  \param f right-hand side
     *  \param r_scratch unused (kept for interface compatibility)
     *  \param Au_scratch used as double-buffer for u_new
     *  \param iters number of sweeps
     *  \param omega damping \f$\omega\in(0,1)\f$
     *  \details Fused: one kernel per sweep computes u_new = u + ω D^{-1}(f - Au).
     *  Uses Au_scratch as double-buffer, then deep_copy back. This replaces the old
     *  3-kernel-per-sweep approach (apply_A + subtraction + update) with 1 kernel + 1 copy.
     */
    static void smooth_jacobi(const Level3D& lev, DeviceView& u, const DeviceView& f,
        ...
        // Au_scratch serves as u_new; boundary entries are zero from allocation
        // and never overwritten, matching Dirichlet BCs.
        auto& u_new = Au_scratch;
        for (int it = 0; it < iters; ++it) {
            Kokkos::parallel_for(
                "jacobi_fused",
                Kokkos::MDRangePolicy<Kokkos::Rank<3>>({1, 1, 1}, {nx - 1, ny - 1, nz - 1}),
    ...
    static void restrict_fullweight(const Level3D& fine, const DeviceView& r_f,
        ...
        if (coarse.N() < SERIAL_THRESHOLD) {
            restrict_serial(fine, r_f, coarse, f_c);
            return;
        }
        const int nxf = fine.nx, nyf = fine.ny;
        const int nxc = coarse.nx, nyc = coarse.ny, nzc = coarse.nz;
    ...
    static void prolong_add(const Level3D& coarse, const DeviceView& u_c, const Level3D& fine,
                            DeviceView& u_f) {
        if (coarse.N() < SERIAL_THRESHOLD) {
            prolong_add_serial(coarse, u_c, fine, u_f);
            return;
        }
        const int nxc = coarse.nx, nyc = coarse.ny, nzc = coarse.nz;
        const int nxf = fine.nx, nyf = fine.ny;
